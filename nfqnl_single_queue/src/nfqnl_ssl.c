...
{
	struct nfq_handle *h_in_lan; 
	struct nfq_q_handle *qh_in_lan;
	int fd_in_lan; 
	int rv_lan;
	...
	qh_in_lan = bind_queue(h_in_lan, 0, &cb);
	...
	while(1){
        rv_lan = -1;
        temp_mask = read_sd;		
		num = select(fd_in_lan+1, &temp_mask, &dummy_mask, &dummy_mask, NULL);
		if (num > 0 ){
